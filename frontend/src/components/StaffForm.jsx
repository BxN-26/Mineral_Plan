...
const EMPTY = {
  firstname: '', lastname: '', initials: '', contract_h: 0, team_id: '',
...
const StaffForm = ({ initial, teams, functions, onSave, onClose }) => {
  ...
  useEffect(() => {
    setF(initial ? { ...EMPTY, ...initial, team_id: initial.team_id || '', cp_balance: initial.cp_balance ?? 0, rtt_balance: initial.rtt_balance ?? 0, charge_rate: initial.charge_rate ?? 0.45, functions: initial.functions || [], primary_function: initial.primary_function || '', manager_id: initial.manager_id || null } : { ...EMPTY });
  }, [initial]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        ...
        <Field label="Équipe">
          <select value={f.team_id || ''} onChange={e => set('team_id', e.target.value || null)} style={inputSt}>
